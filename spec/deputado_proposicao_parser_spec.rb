...
describe DeputadoProposicaoParser do
  def proposicao_url(deputado, year)
    url = "http://www.camara.gov.br/SitCamaraWS/Proposicoes.asmx/ListarProposicoes?sigla=PL&numero=&ano=#{year}&datApresentacaoIni=&datApresentacaoFim=&autor=&parteNomeAutor=ABELARDO&siglaPartidoAutor=&siglaUFAutor=&generoAutor=&codEstado=&codOrgaoEstado=&emTramitacao=&idTipoAutor="
    stub_request(:get, url)
      .to_return(status: 200, body: File.read('spec/fixtures/proposicoes.xml'))
    url
  end

  let(:deputado) { OpenStruct.new(id: 74_016, nome_parlamentar: 'ABELARDO CAMARINHA', uf: 'SP') }
